...
// (sorted_run_chunk.circom). Board positions are global (24-bit), so the
// chunk pipeline supports boards up to 2^24 slots.
...
template ValidityChunk(C, nC, depth, idBits) {
    ...
    signal input num_ballots;               // GLOBAL board size (24-bit)
    signal input num_voters;
    signal input duplicate_rule_id;
    signal input chunk_base;                // k * C (24-bit)
